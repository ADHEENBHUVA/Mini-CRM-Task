        ...
        <header className="h-16 md:h-20 bg-white/70 dark:bg-[#0B0F19]/90 backdrop-blur-xl border-b border-slate-200 dark:border-[#1E293B] sticky top-0 z-10 transition-colors duration-300">
            <div className="flex h-full items-center justify-between px-4 md:px-8">
                ...
                <div className="flex items-center gap-3 md:gap-6 ml-4">
                    ...
                    <button className="relative p-2 text-slate-400 hover:text-indigo-400 hover:bg-[#1E293B] rounded-full transition-all duration-300">
                        <Bell className="w-6 h-6" />
                        <span className="absolute top-2 right-2 block h-2.5 w-2.5 rounded-full border-2 border-[#0B0F19] bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.8)]"></span>
                    </button>


                    <div className="flex items-center gap-4 cursor-pointer group p-1.5 pr-3 hover:bg-[#1E293B] rounded-full transition-all duration-300">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-600 to-purple-500 p-0.5 shadow-lg shadow-indigo-500/20 group-hover:shadow-indigo-500/40">
                            <div className="w-full h-full rounded-full bg-[#151D2C] flex items-center justify-center">
                                <UserCircle className="w-8 h-8 text-indigo-100" />
                            </div>
                        </div>
                        <div className="hidden sm:block text-left">
                            <p className="text-sm font-bold text-slate-200 group-hover:text-white transition-colors">{user.name || user.email || 'Admin User'}</p>
                            <p className="text-xs font-medium text-slate-400">{user.role || 'Administrator'}</p>
                        </div>
                    </div>
                </div>
