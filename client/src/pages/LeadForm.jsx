...
import { ArrowLeft, Save, Briefcase, User, Mail, Phone, Building } from 'lucide-react';
...
const LeadForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert('Lead created safely!');
            navigate('/leads');
        } catch (error) {
            console.error('Error creating lead', error);
            alert(`Failed API Error: ${error.response?.data?.message || error.message}`);
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input required type="text" name="phone" value={formData.phone} onChange={handleChange} className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-[#151D2C] border border-slate-200 dark:border-[#2A374C] rounded-xl text-slate-900 dark:text-white focus:ring-1 focus:ring-indigo-500 outline-none transition-colors" placeholder="+1 (555) 000-0000" />
                            </div>
                        </div>
                    </div>
